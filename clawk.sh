...
# Extraer código de país (region) del JSON de la página
region_code=$(echo "$source_code" | grep -oP '"region":"\K[^"]+')

# Buscar nombre y bandera del país en countries.json
if [[ -n "$region_code" && -f countries.json ]]; then
    country_info=$(jq -r --arg region "$region_code" '.[] | select(.code == $region) | "\(.name) \(.emoji)"' countries.json)
    if [[ -n "$country_info" ]]; then
        region="$country_info ($region_code)"
    else
        region="Desconocido (código: $region_code)"
    fi
else
